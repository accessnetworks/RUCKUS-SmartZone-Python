...
class vSZ_calls:
	...
	def getToken(self, host, username, password):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/serviceTicket"
		body = {'username': username,'password': password} 
	...
	def createDomain(self, host, domain, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/domains?serviceTicket=" + token
   		body = {
                "name": domain,
                "domainType": "PARTNER"
               }
	...
	def createZone(self, host, domainID, zone, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones?serviceTicket=" + token
   		body = {
                "domainId": domainID,
                "name": zone,
                "description": "",
	...
	def getZoneID(self, host, zone, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones?listSize=500&serviceTicket=" + token
		r = requests.get(url, verify=False)	
		for item in r.json()['list']:
			if item['name'] == zone:
				zoneID = item['id']
	...
	def createProxyAAA(self, host, domainID, proxyAAAname, proxyAAAipAddress, sharedSecret, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/services/auth/radius?serviceTicket=" + token
		body = {
                    "domainId": domainID,
                    "name": proxyAAAname,
                    "friendlyName": "",
	...
	def getProxyAAAid(self, host, domainID, proxyAAAname, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/services/auth/query?serviceTicket=" + token
		body = {
                "filters": [
                   {
                     "type": "DOMAIN",
	...
	def queryProxyAAA(self, host, proxyAAAid, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/services/auth/radius/" + proxyAAAid + "?serviceTicket=" + token
		r = requests.get(url, verify=False)	
		return r.json()

	# Create Wifi operator
	def createWifiOperator(self, host, domainID, operatorName, operatorFriendlyName, operatorDomain, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/operators?serviceTicket=" + token
		body = {
                 "domainId": domainID,
                 "name": operatorName,
                 "description": "Citymesh in Belgium",
                 "domainNames": [
                    operatorDomain
                 ],
                 "friendlyNames": [
	...
	def getWifiOperatorID(self, host, domainID, operatorName, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/operators/query?serviceTicket=" + token
		body = {
                "filters": [
                   {
                     "type": "DOMAIN",
	...
	def queryWifiOperator(self, host, operatorID, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/operators/" + operatorID + "?serviceTicket=" + token
		r = requests.get(url, verify=False)	
		return r.json()

	# Create identity provider
	def createIdentityProvider(self, host, domainID, IdentityProviderName, realmName, homeOis, homeOisName, proxyAAAname, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/identityproviders?serviceTicket=" + token
		body = {
    			"domainId": domainID,
	...
	def getIdentityProviderID(self, host, domainID, IdentityProviderName, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/identityproviders/query?serviceTicket=" + token
		body = {
                "filters": [
                   {
                     "type": "DOMAIN",
	...
	def queryIdentityProvider(self, host, identityProviderID, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/identityproviders/" + identityProviderID + "?serviceTicket=" + token
		r = requests.get(url, verify=False)	
		return r.json()
	...
	def createHS20zoneProfile(self, host, zoneID, zoneProfileName, wifiOperatorID, identityProviderID, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/hs20s?serviceTicket=" + token
		body = {
	...
	def getHS20zoneProfile(self, host, zoneID, HS20zoneProfileID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/hs20s/" + HS20zoneProfileID + "?serviceTicket=" + token
		r = requests.get(url, verify=False)
		return r.json()

	# Create HS20 wlan
	def createHS20wlan(self, host, zoneID, wlanName, ssid, HS20zoneProfileID, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlans/hotspot20?serviceTicket=" + token
		body = {
				"name": wlanName,
  				"ssid": ssid,
  				"description": "",
	...
	def getWLANid(self, host, zoneID, wlanName, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlans?listSize=500&serviceTicket=" + token
		r = requests.get(url, verify=False)
		for item in r.json()['list']:
	...
	def getWLAN(self, host, zoneID, wlanID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlans/" + wlanID + "?serviceTicket=" + token
		r = requests.get(url, verify=False)
		return r.json()

    # Create FIPS WPA wlan
	def createWPAwlan(self, host, zoneID, wlanName, ssid, passphrase, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlans?serviceTicket=" + token
		body = {
                "name": wlanName,
                "ssid": ssid,
                "encryption": {
	...
	def createWISPRwlan(self, host, zoneID, wlanName, ssid, passphrase, wisprPortalID, proxyAaaID, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlans/wispr?serviceTicket=" + token
		body = {
                  "name": wlanName,
                  "ssid": ssid,
                  "description": "",
	...
	def createWISPRportal(self, host, zoneID, WISPRportalName, WISPRportalURL, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/portals/hotspot/external?serviceTicket=" + token
		body = {
	...
	def getWISPRportalID(self, host, zoneID, WISPRportalName, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/portals/hotspot?serviceTicket=" + token
		r = requests.get(url, verify=False)
		for item in r.json()['list']:
	...
	def getWISPRportal(self, host, zoneID, WISPRportalID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/portals/hotspot/" + WISPRportalID + "?serviceTicket=" + token
		r = requests.get(url, verify=False)
		return r.json()
	...
	def deleteWifiOperator(self, host, wifiOperatorID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/profiles/hs20/operators/" + wifiOperatorID + "?serviceTicket=" + token
		r = requests.delete(url, verify=False)
		return r
	...
	def deleteZone(self, host, zoneID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "?serviceTicket=" + token
		r = requests.delete(url, verify=False)
		return r

	# Delete domain
	def deleteDomain(self, host, domainID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/domains/" + domainID + "?serviceTicket=" + token
		r = requests.delete(url, verify=False)
		return r

	# Create APgroup
	def createAPgroup(self, host, zoneID, APgroupName, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/apgroups?serviceTicket=" + token
		body = {
  				"name": APgroupName,
 				"description": ""
			}
	...
	def createWLANgroup(self, host, zoneID, WLANgroupName, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlangroups?serviceTicket=" + token
		body = {
  				"name": WLANgroupName,
 				"description": ""
			}
	...
	def getWLANgroupID(self, host, zoneID, WLANgroupName, token):
 		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlangroups?listSize=500&serviceTicket=" + token
		r = requests.get(url, verify=False)	
		for item in r.json()['list']:
	...
	def addMemberToWlanGroup(self, host, zoneID, WLANgroupID, wlanID, vlanID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlangroups/" + WLANgroupID + "/members" + "?serviceTicket=" + token
		body = {
				"id": wlanID,
				"accessVlan": vlanID
			}
	...
	def removeMemberFromWlanGroup(self, host, zoneID, WLANgroupID, wlanID, token):
		url = "https://" + host + ":8443" + "/wsg/api/public/v8_1/rkszones/" + zoneID + "/wlangroups/" + WLANgroupID + "/members/" + wlanID + "?serviceTicket=" + token
		r = requests.delete(url, verify=False)
		return r
